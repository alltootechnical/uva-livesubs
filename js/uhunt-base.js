...
.factory('uhunt_util', function ($http, $timeout, $filter) {
  ...
  return {
    ...
    format_ago: function (t, back) {
      ...
      if (back<=5 && w < 60) { return Math.ceil(w) + ' sec'; }
      if (back<=4 && w < 60*60) { return Math.floor(w/60) + ' min'; }
      if (back<=3 && w < 24*60*60) { return Math.floor(w/60/60) + ' h'; }
      if (back<=2 && w < 30*24*60*60) { return Math.floor(w/60/60/24) + ' d'; }
      if (back<=1 && w < 365*24*60*60) { return Math.floor(w/60/60/24/30) + ' m'; }
...
.factory('uhunt_config', function (uhunt_util, uhunt_create_user) {
  ...
  return {
    ...
    'language_map': {
      ...
      5: { name: 'C++11',  color: 'black' },
      6: { name: 'Python',  color: 'black' },
    },
...
.directive('uhuntLanguage', function (uhunt) {
  return {
    template: '<span ng-switch on="language"><span ng-switch-when="ANSI C"><span style="font-stretch:condensed">ANSI</span> C</span>\
    <span ng-switch-when="C++11">C++<span style="font-stretch:condensed">11</span></span>\
    <span ng-switch-when="Python"><span style="font-stretch:condensed">Python</span></span>\
    <span ng-switch-when="Pascal"><span style="font-stretch:condensed">Pascal</span></span>\
    <span ng-switch-default>{{language}}</span></span>',
...
.directive('uhuntRuntime', function (uhunt_util) {
  return {
    template: '<span>{{runtime_10s}}<span style="vertical-align:4.5px; font-size:15px">{{runtime_1000s}}</span></span>',
    link: function (scope, element, attrs) {
      scope.runtime = uhunt_util.format_ms(uhunt_util.parseInt(attrs.uhuntRuntime));
	  scope.runtime_10s = scope.runtime.toString().substring(0,scope.runtime.toString().indexOf(".")+2);
      scope.runtime_1000s = scope.runtime.toString().substring(scope.runtime.toString().indexOf(".")+2,scope.runtime.toString().length);
    }
  };
})
...
.directive('uhuntRank', function () {
  return {
    template: '<span ng-if="rank==-1">&mdash;</span>\
    <span ng-if="rank>=1000"><span style="font-stretch:condensed">{{rank}}</span></span>\
    <span ng-if="0<rank && rank<1000">{{rank}}</span>',
    link: function (scope, element, attrs) {
      scope.rank = attrs.uhuntRank;
    }
  };
})
...
.directive('uhuntNameUname', function () {
  return {
    ...
    template: '<a class="ellipsis" style="margin-left:5px; width:{{width}}">{{name}} <span style="font-stretch: condensed">({{uname}})</span></a>',
  };
})
...
.directive('uhuntProblemTitle', function (uhunt_problems, uhunt) {
  return {
    ...
    template:
      '<a class="ellipsis" style="margin-left:5px; width:{{width}}; color:white; font-stretch: condensed" target="_blank" ' +
